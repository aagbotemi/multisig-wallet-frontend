...
function NewProposalMd({
  ...
  const { data, write, isError, error } = useContractWrite({
    ...
    args: [
      ...
      valueOfEther == ""
        ...
        : ethers.utils.parseEther(valueOfEther),
      hasTransactionData ? functionSigArgs : "",
      proposalName,
    ],
    chainId: CHAIN_ID,
    overrides: {
  ...
  return (
    <div className="">
      <AppModal>
        <AppModal.Cover
          isOpen={isNewProposalMd}
          setIsOpen={setIsNewProposalMd}
          variant={"md"}
        >
          <AppModal.Header
            showCloseBtn={true}
                  ...
                  <Fragment>
                    <div className="grid grid-cols-2">
                      ...
                      {selectList.map((singleselect, index) => (
                        <div
                          key={index}
                          className="services flex parameter__holders"
                        >
                            ...
                            <div className="flex">
                              ...
                              <div className="form-group ml-4">
                                <input
                                  ...
                                  placeholder="Value Of Ether"
                                  onChange={(e) =>
                                    setValueOfEther(e.target.value)
                                  }
                                />
                              </div>
                            </div>

                            {selectList.length - 1 === index &&
                              selectList.length < 20 && (
                                <button
                                  type="button"
    ...
    <BottomSheet>
      ...
      >
        ...
        <div className="wf-100 py-12 px-7 fixed bottom-0 flex justify-between">
          <button className="bg-primary pdy-16 round-8 text-white wf-100 fs-16 lh-16 fw-normal mgt-20">
            Close
          </button>
          <button className="bg-primary pdy-16 round-8 text-white wf-100 fs-16 lh-16 fw-normal mgt-20">
            Create
